...
internal class FlatMapOrIncidentTest : UnitTest() {

    init {

        "The  extension function `flatMapOrIncident` of the `JDBCResult` type" - {

            "when a variable has the success value" - {
                val original: JDBCResult<String> = createResult(ORIGINAL_VALUE.asSuccess())
                val result: TransactionResult<Int, Unit> = original.flatMapOrIncident { result ->
