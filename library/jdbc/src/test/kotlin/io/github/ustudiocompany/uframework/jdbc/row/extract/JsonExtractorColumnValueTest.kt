...
import io.github.ustudiocompany.uframework.jdbc.row.extractor.getJson
import io.github.ustudiocompany.uframework.jdbc.sql.ColumnLabel
import io.kotest.datatest.withData
import io.kotest.matchers.shouldBe
import io.kotest.matchers.types.shouldBeInstanceOf

internal class JsonExtractorColumnValueTest : AbstractExtractorColumnValueTest() {
    ...
    init {
        ...
        "The `getJson` function" - {
            ...
            "when column index is valid" - {
                withData(
                    ...
                    withData(
                        ...
                        executeQuery(selectSql) {
                            getJson(metadata.columnIndex).shouldBeSuccess(value)
                        }
                    }
                }
                ...
                withData(
                    ...
                    executeQuery(makeSelectEmptyRowSql()) {
                        val failure = getJson(metadata.columnIndex).shouldBeFailure()
            ...
            "when column index is invalid then function should return an error" {
                ...
                executeQuery(makeSelectEmptyRowSql()) {
                    val error = getJson(INVALID_COLUMN_INDEX).shouldBeFailure()
            ...
            "when column name is valid" - {
                withData(
                    ...
                    withData(
                        ...
                        val selectSql = MultiColumnTable.makeSelectAllColumnsSql(rowId)
                        executeQuery(selectSql) {
                            getJson(metadata.columnName).shouldBeSuccess(value)
                        }
                    }
                }
                ...
                withData(
                    ...
                    executeQuery(makeSelectEmptyRowSql()) {
                        val failure = getJson(metadata.columnName).shouldBeFailure()
                        val cause = failure.cause.shouldBeInstanceOf<JDBCErrors.Row.TypeMismatch>()
            ...
            "when column name is invalid then function should return an error" {
                ...
                executeQuery(makeSelectEmptyRowSql()) {
                    val error = getJson(INVALID_COLUMN_NAME).shouldBeFailure()
                    val cause = error.cause.shouldBeInstanceOf<JDBCErrors.Row.UndefinedColumn>()
