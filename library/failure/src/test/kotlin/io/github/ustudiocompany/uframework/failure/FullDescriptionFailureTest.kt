...
internal class FullDescriptionFailureTest : UnitTest() {

    init {

        "Tee `fullDescription` function" - {
            withData(
                nameFn = { (failure, _) -> failure.toString() },
                listOf(
                    failure(code = CODE_1, description = DES_1) to DES_1,
                    failure(
                        code = CODE_2,
                        description = DES_2,
                        cause = failure(
                    ...
                    ) to "$DES_3 $DES_2 $DES_1"
                )
            ) { (failure, expected) ->
                failure.fullDescription() shouldBe expected
            }
        }
    }
    ...
    private fun failure(code: String, description: String, cause: Failure): Failure =
        Child(code = code, cause = Failure.Cause.Failure(cause), description = description)
